...
class TextRedirector:
    ...
    def write(self, message):
        if message:
            self.gui.root.after(0, lambda m=message: self.gui.append_log(m.rstrip()))
...
class OsuRelaxGui:
    def __init__(self, root):
        ...
        self.stderr_redirector = TextRedirector(self)
        ...
        self.build_ui()
        self.refresh_status("Waiting for osu! or a beatmap selection.")
        self.root.after(1000, self.poll_osu_state)

    def build_ui(self):
        main = ttk.Frame(self.root, padding=16)
        main.pack(fill="both", expand=True)

        ttk.Label(main, text="osu! Relax", font=("Segoe UI", 18, "bold")).pack(anchor="w")
        ttk.Label(main, text="Load a beatmap and start the automation from the comfort of a window.", wraplength=680).pack(anchor="w", pady=(4, 12))

        beatmap_row = ttk.Frame(main)
        beatmap_row.pack(fill="x", pady=(0, 8))
        ttk.Label(beatmap_row, text="Beatmap:").pack(side="left")
        self.beatmap_var = tk.StringVar()
        self.beatmap_entry = ttk.Entry(beatmap_row, textvariable=self.beatmap_var, width=70)
        self.beatmap_entry.pack(side="left", padx=(8, 8), fill="x", expand=True)
        ttk.Button(beatmap_row, text="Browse", command=self.browse_beatmap).pack(side="left")
        ...
        ttk.Label(main, textvariable=self.status_var, foreground="#1f5fbf").pack(anchor="w", pady=(2, 8))

        self.log_area = ScrolledText(main, height=16, wrap="word")
        self.log_area.pack(fill="both", expand=True)
        self.log_area.configure(state="disabled")

        self.set_default_beatmap()

    def set_default_beatmap(self):
        recent = find_recent_beatmap()
        if recent:
            self.beatmap_var.set(recent)

    def browse_beatmap(self):
        path = filedialog.askopenfilename(
            title="Select an osu! beatmap",
            filetypes=[("osu! beatmap", "*.osu")],
            initialdir=os.path.join(os.environ.get("LOCALAPPDATA", ""), "osu!", "Songs") if os.environ.get("LOCALAPPDATA") else None,
        )
        if path:
            self.beatmap_var.set(path)
    ...
    def start(self):
        beatmap_path = self.beatmap_var.get().strip() or find_recent_beatmap()
        if not beatmap_path:
            messagebox.showerror("Missing beatmap", "Select a .osu file or open osu! so the latest map can be detected.")
            return

        if self.engine and self.engine.is_running:
            messagebox.showinfo("Already running", "The automation is already active.")
            return

        self.refresh_status("Loading beatmap...")
        try:
            self.engine = OsuRelaxCheatsEngine(beatmap_path)
        except Exception as exc:
            messagebox.showerror("Beatmap error", f"Could not load the beatmap: {exc}")
            self.refresh_status("Failed to load beatmap")
            return

        self.append_log(f"[+] Loaded {os.path.basename(beatmap_path)} ({len(self.engine.hit_objects)} objects)")
        ...
        if h_process:
            self.append_log(f"[+] Connected to osu! (PID: {pid})")
            self.time_addr, _ = scan_osu_memory(h_process)
            if self.time_addr:
                self.append_log(f"[+] Memory sync active — game time: 0x{self.time_addr:08X}")
            else:
                self.append_log("[!] Using wall-clock sync")
        else:
            self.append_log("[!] osu! is not running yet; the script will wait for it")

        if self.h_process:
            time_reader = make_time_reader(self.h_process, self.time_addr) if self.time_addr else None
        else:
        ...
        if self.time_addr:
            self.engine.start(time_reader, start_key='e', key_1='z', key_2='x')
        else:
            self.engine.start(time_reader, start_key='e', key_1='z', key_2='x')

        self.refresh_status("Automation running")
    ...
    def poll_osu_state(self):
        if self.engine and not self.engine.is_running:
            self.refresh_status("Automation finished or stopped")
            self.start_button.config(state="normal")
            self.stop_button.config(state="normal")
            self.engine = None
            if self.h_process:
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1].endswith('.osu') and os.path.isfile(sys.argv[1]):
        root = tk.Tk()
        app = OsuRelaxGui(root)
        app.beatmap_var.set(sys.argv[1])
        app.start()
        root.mainloop()
    else:
        root = tk.Tk()
